...
## 📋 RESUMEN EJECUTIVO *(para Notion / seguimiento de proyecto)*

### ✅ UI / Navegación
- **Bug selector Área+Sector corregido**: El `UnifiedAreaSectorSelector` no navegaba directamente al seleccionar "Mant > Planta Viguetas" (o cualquier cambio de área). Bug: el `useEffect` de sincronización en Sidebar.tsx tenía `currentArea?.name` en sus deps, lo que causaba que al llamar `setArea()` el efecto se disparara con el pathname viejo y REVERTIERA el área antes de que la navegación completara. Fix: removido `currentArea?.name` de los deps (el efecto solo debe correr cuando cambia `pathname`). Además se hicieron las comparaciones de nombre de área case-insensitive para robustez con datos de DB.


### ✅ Seguridad / Permisos
- **Protección de rutas API criticas**: Se agregaron chequeos de permisos (`requirePermission` / `hasUserPermission`) a las rutas API de escritura (POST/PUT/DELETE) en Compras, Mantenimiento, Nominas y Almacen. Esto impide que usuarios sin los permisos correspondientes puedan crear, editar o eliminar registros desde la API.
...
### ✅ Usuarios / Admin / Configuracion (permisos)
...
| Modulo | Estado | Nota |
|--------|--------|------|
| Compras (permisos API) | ✅ Completo | proveedores, comprobantes, ordenes-compra, pedidos, solicitudes protegidos |
| Work Orders (permisos API) | ✅ Ya protegido | Usa withGuards con requiredPermissions |
| Failure Occurrences (permisos API) | ✅ Completo | route.ts POST, [id] PATCH/DELETE, create-work-order, quick-report protegidos |
| Tesoreria (permisos API) | ✅ Ya protegido | Usa requirePermission de tesoreria/auth |
| Nominas (permisos API) | ✅ Completo | empleados, corridas, gremios, periodos, sectores, categorias, componentes protegidos |
| Almacen (permisos API) | ✅ Completo | requests, despachos, devoluciones protegidos |
| Produccion (permisos API) | ⏭ No aplica | Las rutas no existen aun |
| Mantenimiento (permisos FE+BE) | ✅ Completo | machines.*, work_orders.*, preventive_maintenance.*, tasks.*, fixed_tasks.* conectados |
| Usuarios (permisos FE+BE) | ✅ Completo | users.view/create/edit/delete/edit_role/activate_deactivate conectados |
| Empresas (permisos BE) | ✅ Completo | companies.view/create/edit/delete/manage_users conectados |
| Configuracion (permisos FE+BE) | ✅ Completo | settings.view/edit/system, notifications.manage conectados |
| Admin Permisos/Roles (permisos FE+BE) | ✅ Completo | admin.permissions/admin.roles conectados |
| Reportes (permisos BE) | ✅ Completo | reports.view/advanced conectados |
| QR (permisos BE) | ✅ Completo | qr.view/generate conectados |
| Sectores (permisos BE) | ✅ Completo | sectors.create/edit/delete conectados |
| Auditoria (permisos FE+BE) | ✅ Completo | audit.view/export conectados |
| Panol / Tools (permisos FE+BE) | ✅ Completo | tools.*/panol.* conectados en 25 API routes + 4 paginas frontend |
| SAFETY: PTW (permisos FE+BE) | ✅ Completo | ptw.* conectados en backend (view/create/approve/reject/etc) + frontend ya tenia checks |
| SAFETY: LOTO (permisos FE+BE) | ✅ Completo | loto.* conectados en backend + frontend ya tenia checks |
| SAFETY: MOC (permisos FE+BE) | ✅ Completo | moc.* en backend + frontend (MOCList, MOCDetail) con hasPermission |
| SAFETY: Skills (permisos FE+BE) | ✅ Completo | skills.*/certifications.* en backend (requirePermission) + frontend (hasPermission props) |
| SAFETY: Calibracion (permisos FE+BE) | ✅ Completo | calibration.* en backend + frontend (create dialog) |
| SAFETY: Contratistas (permisos FE+BE) | ✅ Completo | contractors.* en backend + frontend (create dialog) |
| SAFETY: Lubricacion (permisos FE+BE) | ✅ Completo | lubrication.* en backend + frontend (create dialog) |
| SAFETY: Monitoreo (permisos FE+BE) | ✅ Completo | condition_monitoring.* en backend + frontend (create dialog) |
| SAFETY: Conocimiento (permisos FE+BE) | ✅ Completo | knowledge.* en backend + frontend (create dialog) |
| SAFETY: Contadores (permisos FE+BE) | ✅ Completo | counters.* en backend + frontend (CounterPanel, CounterCard) |
| Tesoreria (permisos FE) | ✅ Completo | treasury.* conectados en 10 paginas frontend (layout, page, cajas, bancos, cheques, transferencias, conciliacion, depositos, cierres, movimientos) |
| Almacen (permisos FE+BE granular) | ✅ Completo | almacen.dispatch.*/request.*/return.*/transfer/adjust/reservation.*/manage_warehouses/manage_locations/manage_all/view_costs conectados en 6 tabs frontend + 15 API routes backend |
| Compras Cotizaciones (permisos FE+BE) | ✅ Completo | compras.cotizaciones.view/create/edit/delete/seleccionar/convertir_oc conectados en 6 API routes + detail modal frontend |
| Compras Pedidos (permisos FE+BE) | ✅ Completo | compras.pedidos.view/create/edit/delete/enviar/aprobar/rechazar conectados en 7 API routes + list + detail modal frontend |
| Compras Granular (permisos FE) | ✅ Completo | 48 permisos granulares en 15 paginas: 9 ya tenian checks, 6 corregidas/agregadas (proveedores/[id], stock/ajustes, stock/transferencias, carga-masiva, recepciones, solicitudes/editar). Permission strings corregidos (stock.create/edit/delete -> stock.ajustes/transferencias) |
| Produccion (permisos FE extras) | ✅ Completo | produccion.ordenes.release, partes.confirm/review, config.routines, reportes.export conectados en 4 paginas frontend |
| **AUDIT MASIVO: 213 API routes sin auth** | ✅ Completo | Auditoria exhaustiva de 1,067 route.ts. 213 rutas tenian ZERO auth (accesibles por internet). Todas protegidas con `requireAuth`. Incluye: employees/salary (14), costos (35), maintenance (21), products/inputs/insumos (27), areas/zones (5), assets (4), plant ops (5), dashboards, AI/assistant, mobile, work-orders, etc. |
| **AUDIT MASIVO: 6 paginas frontend sin guard** | ✅ Completo | maquinas (page+layout), mobile/reporte-rapido, mobile/mi-dia, ai/demand-forecast, nueva-empresa, cuenta/billing — todas protegidas |
| **Permisos granulares Panol (FE)** | ✅ Completo | 8 paginas auditadas, stock-in/stock-out buttons + handlers protegidos con usePanolPermissions |
| **Permisos granulares Nominas (FE)** | ✅ Completo | 6 paginas: empleados, gremios, categorias, sectores, componentes — canManageNominas en todos los botones |
| **Permisos granulares Produccion config (FE)** | ✅ Completo | 5 paginas: centros-trabajo, codigos-motivo, recursos, turnos, ordenes/nueva — permisos granulares produccion.config.* |
| Ventas (permisos FE extras) | ✅ Completo | cotizaciones.duplicate/version, entregas.dispatch/program, facturas.send, pagos.apply, descuentos.apply/unlimited, ordenes.edit/delete, aprobaciones.approve/reject, reportes.rentabilidad conectados en 9 componentes |
| Ventas (Legacy audit) | ✅ Documentado | 17 permisos ALL_CAPS legacy mapeados; se mantienen como aliases en catalog |
| Frontend pages sin guard | ✅ Completo | 7 paginas protegidas: maquinas (page+layout), reporte-rapido, mi-dia, demand-forecast, nueva-empresa, billing |

---
---

## 🛠️ LOG TECNICO *(para desarrollo / Claude Code)*

### Session Overview
- **Date**: 2026-02-27
- **Focus Area**: Seguridad — agregar permission checks a rutas API criticas
...
### Decisions Made
- **Decision**: For routes that already have auth (getUserFromToken, getPayrollAuth, verifyToken), add `hasUserPermission` AFTER the existing auth check — **Reason**: Minimally invasive, preserves existing auth flow, adds permission as a second layer
- **Decision**: For almacen routes with NO existing auth, use `requirePermission` from shared-helpers — **Reason**: These routes had zero auth, so adding the full pattern (auth + permission in one call) is the right approach
- **Decision**: Use `ingresar_compras` as gate for ALL compras writes — **Reason**: No granular compras permissions defined yet, per task instructions
- **Decision**: Skip production routes — **Reason**: No route files exist under app/api/produccion/

### Issues Encountered
- **Issue**: Almacen routes (requests, despachos, devoluciones) had NO authentication at all on POST handlers, only taking companyId from request body — **Resolution**: Added `requirePermission` from shared-helpers which handles both auth and permission check

### Skills Used
- orvit-permissions — applied permission check patterns from existing codebase

---

### Session 2 — Conexion de permisos del modulo Mantenimiento
...
### Decisions Made
- **Decision**: Update all old Spanish permission names (crear_maquina, editar_maquina, etc.) to standard module.action format (machines.create, machines.edit) — **Reason**: The old names were never enforced in the backend; the new format matches the 402 permissions defined in the system
- **Decision**: Use `hasAnyPermission(['new_name', 'old_name'])` for navigation hooks — **Reason**: Roles in DB may still have old permission names; this provides backward compatibility while transitioning
- **Decision**: Add `requirePermission` before existing auth in backend routes rather than replacing it — **Reason**: Minimally invasive; the existing auth handles company boundary checks and user resolution that requirePermission doesn't replicate
- **Decision**: Fixed tasks backend already had correct hasPermission checks using `fixed_tasks.create/edit/delete` — **Reason**: Confirmed working, skipped redundant changes
...
### Session 3 — Conexion de permisos USERS, ADMIN, CONFIG
...
### Decisions Made
- **Decision**: Add `requirePermission` before existing auth checks in API routes rather than replacing them — **Reason**: Minimally invasive, preserves existing auth flow (company boundary checks, user resolution), adds permission as a first layer
- **Decision**: Keep backward compatibility with `canManageUsers` (`gestionar_usuarios`) as fallback — **Reason**: Roles in DB may still use old permission names; `||` with new granular permissions ensures both old and new work
- **Decision**: Use `disabled` prop on save buttons rather than hiding them — **Reason**: Better UX, user can see the button exists but understand they lack permission
...
### Session 4 — Conexion de permisos PANOL / TOOLS (frontend + backend)
...
- [x] **Backend API routes (25 handlers)**: Replaced local JWT auth helpers (cookies/verifyToken/jwtVerify/getCurrentUser/getCurrentUserId) with `requirePermission` from `@/lib/auth/shared-helpers` across ALL tools and panol API routes.
  - `tools/route.ts`: GET→`tools.view`, POST→`tools.create`, PUT→`tools.edit`
  - `tools/[id]/route.ts`: GET→`tools.view`, PUT/PATCH→`tools.edit`, DELETE→`tools.delete`
  - `tools/[id]/qr/route.ts`: GET→`tools.view`
  - `tools/[id]/component-usage/route.ts`: GET→`tools.view`
  - `tools/movements/route.ts`: GET→`panol.view_products`, POST→`panol.register_movement`
  - `tools/loans/route.ts`: GET/POST→`tools.manage_loans`
  - `tools/loans/[id]/return/route.ts`: POST→`tools.manage_loans`
  - `tools/use-stock/route.ts`: POST→`tools.manage_stock`
  - `tools/restock/route.ts`: POST→`tools.manage_stock`
  - `tools/dashboard/route.ts`: GET→`panol.view_products` (was NO auth)
  - `tools/search/route.ts`: GET→`tools.view`
  - `tools/categories/route.ts`: GET→`tools.view`, POST→`tools.create` (was NO auth)
  - `tools/categories/[id]/route.ts`: GET→`tools.view`, PUT→`tools.edit`, DELETE→`tools.delete` (was NO auth)
  - `tools/suppliers/route.ts`: GET→`tools.view`, POST→`tools.create` (was NO auth)
  - `tools/suppliers/[id]/route.ts`: GET→`tools.view`, PUT→`tools.edit`, DELETE→`tools.delete` (was NO auth)
  - `tools/locations/route.ts`: GET→`tools.view`, POST→`tools.create` (was NO auth)
  - `tools/locations/[id]/route.ts`: GET→`tools.view`, PUT→`tools.edit`, DELETE→`tools.delete` (was NO auth)
  - `tools/reservations/route.ts`: GET→`panol.view_products`, POST→`panol.register_movement`
  - `tools/reservations/[id]/route.ts`: GET→`panol.view_products`, PATCH→`panol.register_movement`, DELETE→`tools.delete`
  - `tools/qr-batch/route.ts`: POST→`tools.view`
  - `panol/conteo/route.ts`: GET→`panol.view_products`, POST→`panol.register_movement` (upgraded from requireAuth)
  - `panol/conteo/[id]/route.ts`: GET→`panol.view_products`, PATCH→`panol.register_movement` (upgraded from requireAuth)
  - `panol/conteo/[id]/finalize/route.ts`: POST→`panol.register_movement` (upgraded from requireAuth)
  - `panol/consumption/route.ts`: GET→`panol.view_products`
  - `panol/forecast/route.ts`: GET→`panol.view_products`
  - `panol/lots/route.ts`: GET→`panol.view_products`, POST→`panol.create_product`
  - `panol/lots/installations/route.ts`: GET→`panol.view_products`, POST→`panol.register_movement`
  - `panol/alerts/route.ts`: GET→`panol.view_products`
  - `panol/field-vs-storage/route.ts`: GET→`panol.view_products`
...
- [x] **Frontend pages (4 pages updated)**:
  - `panol/prestamos/page.tsx`: Added `canManageLoans` check — "Devolver" button (mobile + desktop) hidden without permission
  - `panol/movimientos/page.tsx`: Added `canRegisterMovement` check — "Nuevo" and "Registrar primer movimiento" buttons hidden without permission
  - `panol/reservas/page.tsx`: Added `canManageReservations` check — pick/cancel/return action items hidden without permission
  - `panol/rapido/page.tsx`: Added `canManageStock`, `canRegisterMovement`, `canManageReservations` — quick action cards filtered by permission
...
### Decisions Made
- **Decision**: Replace ALL local auth helpers (getCurrentUser, getCurrentUserId, getUserFromToken) with centralized `requirePermission` — **Reason**: Eliminates duplicated JWT code across 20+ files, provides consistent auth + permission checking in one call
- **Decision**: Use `user!.companyId` from requirePermission result instead of query params — **Reason**: Prevents cross-tenant attacks where companyId could be spoofed in request body/params
- **Decision**: Filter quick action cards in rapido page instead of just disabling them — **Reason**: Cleaner UX for users without write permissions; they only see actions they can perform
- **Decision**: Keep `canManageReservations` mapped to `panol.register_movement` — **Reason**: Reservations are part of the movement workflow; a separate `tools.approve_requests` is for approval-only flows

### Issues Encountered
- **Issue**: 8 API routes had ZERO authentication (dashboard, categories, suppliers, locations, component-usage + their [id] variants) — **Resolution**: Added `requirePermission` which handles both auth and permission checking, closing a significant security gap
- **Issue**: The `use-stock/route.ts` imported `createAndSendInstantNotification` that was only in the import but unused in code — **Resolution**: Removed the unused import along with other stale auth imports

### Skills Used
- orvit-permissions — applied permission check patterns
- orvit-api — followed API route patterns (requirePermission at top of handler)
...
### Session 5 — Conexion de permisos SAFETY modules (PTW, LOTO, MOC, Skills, Calibration, Contractors, Lubrication, Condition Monitoring, Knowledge, Counters)
...
### Decisions Made
- **Decision**: For PATCH routes with multiple actions (PTW, LOTO, MOC), determine permission dynamically from the `action` field in the request body — **Reason**: A single PATCH endpoint handles multiple state transitions; each transition requires a different permission
- **Decision**: Skills API routes had their own manual permission checks (querying userOnCompany with role.permissions) — refactored to use centralized `requirePermission` — **Reason**: Standardizes auth pattern, removes 20+ lines of boilerplate per handler
- **Decision**: `UserSkillAssignDialog` is exported but not used on any page yet — no frontend guard needed at this time — **Reason**: Backend protection via requirePermission is sufficient; frontend guard will be added when the dialog is integrated into a page
...
### Session 6 — Conexion de permisos TESORERIA frontend (treasury.*)

### What Was Done

#### Completed
- [x] **Layout guard**: Updated `PermissionGuard` from `ingresar_tesoreria` to `treasury.ingresar` in `tesoreria/layout.tsx`
- [x] **Main tesoreria page**: Added `hasPermission` checks for all shortcut buttons — Cajas (`treasury.manage_cash`), Bancos (`treasury.manage_bank`), Cheques (`treasury.manage_cheque`), Transferencias (`treasury.transfer`), Depositos (`treasury.manage_cash`), Cierres (`treasury.manage_cash`), Conciliacion (`treasury.reconcile`), Flujo de Caja (`treasury.reports`). Shortcuts are filtered — users only see sections they have permission for.
- [x] **Cajas page**: Added `canManageCash` — guards "Nueva Caja" dialog and Ingreso/Egreso action buttons
- [x] **Bancos page**: Added `canManageBank` — guards "Nueva Cuenta" dialog
- [x] **Cheques page**: Added `canManageCheque` — guards action dropdowns (depositar/endosar/anular/cobrar/rechazar)
- [x] **Transferencias page**: Added `canTransfer` — guards "Nueva Transferencia" dialog
- [x] **Conciliacion page**: Added `canReconcile` — guards "Importar Extracto" button, "Cerrar Conciliacion" button, auto-match button, and empty state "Importar primer extracto" button
- [x] **Depositos page**: Added `canManageCash` — guards "Nuevo Deposito" dialog
- [x] **Cierres page**: Added `canManageCash` — guards "Nuevo Cierre" dialog
- [x] **Movimientos page**: Added `canViewReports` — guards "Exportar" button
- [x] **Backend**: Verified all 22+ API route handlers already have `requirePermission` from `@/lib/tesoreria/auth` — skipped
...
### Decisions Made
- **Decision**: Skip backend — all API routes already have `requirePermission` from a custom `tesoreria/auth.ts` module with its own granular `tesoreria.*` permission strings — **Reason**: Backend is already fully protected, adding duplicate checks would be redundant
- **Decision**: Use `.filter(s => s.visible)` on shortcut arrays instead of conditional rendering per item — **Reason**: Cleaner code, avoids JSX clutter when many shortcuts need guards
- **Decision**: Map depositos and cierres to `treasury.manage_cash` — **Reason**: These are cash management operations (deposits/closures) and don't have their own dedicated permission
...
### Session 7 — Conexion de permisos ALMACEN granulares (frontend + backend)
...
- [x] **Backend API routes (almacen/*)**:
  ...
  - `despachos/route.ts` PATCH: Added per-action permission map (prepare/ready→`dispatch.process`, dispatch→`dispatch.confirm`, receive→`dispatch.receive`, cancel→`dispatch.cancel`)
  - `requests/route.ts` GET: Added `requirePermission('almacen.request.view')`
  - `requests/route.ts` PATCH: Added per-action permission map (submit→`request.edit`, approve→`request.approve`, reject→`request.reject`, cancel→`request.cancel`)
  - `devoluciones/route.ts` GET: Added `requirePermission('almacen.return.view')`
  - `devoluciones/route.ts` PATCH: Added per-action permission map (submit→`return.create`, accept/reject→`return.process`)
...
- [x] **Frontend components (6 tabs + 2 pages)**:
  - `DespachosTab.tsx`: Added `canProcess`, `canConfirm`, `canReceive`, `canCancel` — guarded all action buttons (Preparar, Marcar Listo, Despachar, Recibido, Cancelar), PrimaryAction component, batch action, and "Nuevo Despacho" button
  - `SolicitudesTab.tsx`: Added `canEdit`, `canApprove`, `canReject`, `canCancel` — guarded Enviar, Aprobar, Rechazar, Cancelar actions, batch approve, and "Nueva Solicitud" button
  - `DevolucionesTab.tsx`: Added `canProcess` — guarded Aceptar/Rechazar actions
  - `ReservasTab.tsx`: Added `canReleaseReserva` (`almacen.reservation.release`) — guarded batch release button and per-row release dropdown (both mobile and desktop)
  - `TransferenciasTab.tsx`: Added `canTransfer` — guarded Enviar, Recibir, Cancelar actions and "Nueva Transferencia" button
  - `AjustesTab.tsx`: Added `canAdjust` — guarded Confirmar, Aprobar, Rechazar actions and "Nuevo Ajuste" button
  - `app/almacen/page.tsx`: Added `canEditRequest` and `canProcessDispatch` — guarded header "Nueva Solicitud" and "Nuevo Despacho" buttons
  - `app/almacen/configuracion/page.tsx`: Added `canManageAll` (`almacen.manage_all`) — full page guard with "Sin permisos" fallback
...
### Decisions Made
- **Decision**: Use per-action permission maps in PATCH routes — **Reason**: A single PATCH endpoint handles multiple state transitions; each transition requires a different permission (e.g., dispatch.process vs dispatch.confirm)
- **Decision**: Add `requirePermission` BEFORE existing auth in compras/stock/* routes — **Reason**: These routes have their own getUserFromToken; adding permission as a first check prevents unnecessary DB queries for unauthorized users
- **Decision**: Rename permission variable to `canReleaseReserva` in ReservasTab — **Reason**: The component already had a local `canRelease` variable per-row; using a different name avoids shadowing
...
### Session 8 — Conexion de permisos COMPRAS Cotizaciones + Pedidos (frontend faltantes)
...
### Session 8 — Conexion de permisos remaining Produccion + Ventas extras (frontend only)
...
### Session 9 — Proteccion de paginas frontend sin guard (PermissionGuard + role checks)
...
- [x] **`/app/nueva-empresa/page.tsx`** (MEDIUM RISK): Added role check — only `ADMIN_ENTERPRISE` and `SUPERADMIN` can access. Shows "No autorizado" with redirect button for other roles.
- [x] **`/app/cuenta/billing/page.tsx`** (MEDIUM RISK): Added `useAuth` + `isAdmin` flag (ADMIN_ENTERPRISE/SUPERADMIN/ADMIN). Gated subscription-modifying actions: cancel subscription button, reactivate button, apply coupon button, configure auto-payment button — all hidden for non-admin users. Billing info remains visible to all authenticated users.
...
### Decisions Made
- **Decision**: Allow SUPERADMIN in addition to ADMIN_ENTERPRISE for nueva-empresa page — **Reason**: SUPERADMIN is the platform owner and should always be able to create companies
- **Decision**: Keep billing page visible to all authenticated users but gate modifying actions behind isAdmin — **Reason**: Users should be able to see their plan info, but only admins should modify subscriptions
- **Decision**: Include ADMIN role in billing isAdmin check — **Reason**: ADMIN is a system role that should have administrative access to billing alongside ADMIN_ENTERPRISE and SUPERADMIN
...
### Session: Compras Module — Granular Permission Checks on Action Buttons
...
- [x] Fixed `proveedores/[id]/page.tsx` — added `useAuth` import and gated Edit button with `compras.proveedores.edit`
- [x] Fixed `stock/ajustes/page.tsx` — corrected invalid permission strings from `compras.stock.create/edit/delete` (not in Permission type) to `compras.stock.ajustes`
- [x] Fixed `stock/transferencias/page.tsx` — corrected invalid permission strings from `compras.stock.create/edit/delete` to `compras.stock.transferencias`
- [x] Fixed `comprobantes/carga-masiva/page.tsx` — added `useAuth` import, gated "Guardar Aprobadas" with `canCreate`, disabled "Guardar Todo" without `compras.comprobantes.create`
- [x] Fixed `recepciones/page.tsx` — added `useAuth` import, gated "Nueva Recepcion", edit, confirm, and delete actions with `compras.ordenes.edit`
- [x] Fixed `solicitudes/[id]/editar/page.tsx` — added `useAuth` import, disabled "Guardar Cambios" button without `compras.solicitudes.edit`
...
### Decisions Made
- **Decision**: Use `compras.ordenes.edit` for recepciones actions — **Reason**: Reception is part of the purchase order workflow, and the user specified this mapping
- **Decision**: For stock pages, use single permission `compras.stock.ajustes` and `compras.stock.transferencias` for all actions (create/edit/delete) — **Reason**: The Permission type only defines these two granular stock permissions, not CRUD-level ones

### Skills Used
- orvit-permissions — audited and applied permission check patterns across 15 pages

---

### Session 10 — Auditoria de seguridad completa + proteccion masiva de rutas API (213 rutas sin auth)
...
#### Fixes aplicados — Backend (213 rutas protegidas)

**P0 — Criticos** (ya estaban protegidos de sesiones previas, verificados 35/37 OK):
- `machine-order` y `data-standards`: Agregado `requireAuth` (unicos 2 faltantes)

**P1 — Employee/Salary/Cost routes (49 archivos)**:
...
**P2 — Maintenance/Products/Inputs (52 archivos)**:
...
**P3 — Remaining routes (71 archivos)**:
...
**P4 — 7 paginas sin guard protegidas**:
...
**P5a — Panol granular (8 paginas auditadas)**:
...
**P5b — Compras granular (15 paginas auditadas)**:
- 9 ya OK, 6 arregladas (proveedores/[id], stock/ajustes, stock/transferencias, carga-masiva, recepciones, solicitudes/editar)
- Bug encontrado y corregido: stock pages usaban `compras.stock.create/edit/delete` que NO existen en Permission type → cambiado a `compras.stock.ajustes/transferencias`

**P5c — Nominas granular (6 paginas)**:
...
**P5d — Produccion config (5 paginas)**:
...
### Skills Used
- orvit-permissions — applied permission check patterns at scale

### ✅ Mantenimiento Correctivo / Incidentes
- **Formulario "Ya resuelta" rediseñado como wizard de 4 pasos** en `CreateIncidentDialog.tsx`:
  - Paso 1 — Incidente: máquina, componente, qué pasó, causó parada, descripción detallada
  - Paso 2 — Resolución: diagnóstico, solución, tipo de fix (Definitiva/Parche), resultado, tiempo, herramientas, repuestos
  - Paso 3 — Equipo: supervisor (default user actual) + operadores (MultiSelect)
  - Paso 4 — Controles de seguimiento con 2 tipos: "desde resolución" (aislado) y "del control anterior" (encadenado)
- **Herramientas y Repuestos**: Combobox multi-selección con filtrado en tiempo real, checkboxes visuales, auto-focus en campo de cantidad tras seleccionar item, teclado numérico en móvil (`inputMode="numeric"`)
- **API `quick-report`**: Actualizada para recibir `performedByIds`, `fixType`, `controlPlan` con campo `type` (from_resolution/from_previous). Lógica de scheduling diferenciada por tipo de control
- **Datos de ejemplo**: 42 items creados (16 HAND_TOOL, 20 SPARE_PART, 6 CONSUMABLE) para testing en empresa Pretensados Córdoba

- **FailureQuickReportDialog convertido a stepper wizard** (3 pasos):
  - Paso 1 — Equipo: máquina/componente (ComponentTreeSelector)
  - Paso 2 — Problema: título, síntomas, downtime, descripción detallada
  - Paso 3 — Detalle: foto prominente (zona dashed), flags (intermitente/seguridad/observación)
  - Eliminado "Ya lo resolví" del paso 3 (para eso existe el flujo "Ya resuelta" en CreateIncidentDialog)
  - Sticky header con Stepper + sticky footer con navegación
  - Validación por paso (máquina en paso 1, título en paso 2)
- **AssignAndPlanDialog mejorado**:
  - Header con OT# badge + título + máquina + X custom
  - Responsable ahora es Combobox buscable (en lugar de Select básico) con avatars de iniciales
  - Prioridad con dots de color (rojo P1, naranja P2, amarillo P3, azul P4)
  - SLA compacto en una línea
  - Secciones visuales: "ASIGNACIÓN" y "PLANIFICACIÓN"
  - Preview de resultado en footer: "Se asignará a **Nombre** como **Pendiente/Programada/En Progreso**"
- **GuidedCloseDialog — Corrección de diagnóstico al cerrar OT**:
  - Sección "Lo que se reportó" muestra el reporte original de la falla (título, descripción, máquina, componente, tipo)
  - Toggle "¿Coincide con lo que encontraste?" — si NO coincide, aparece campo "¿Qué era realmente?"
  - `confirmedCause` movido al cierre mínimo (antes solo en profesional)
  - Dato guardado en SolutionApplied.confirmedCause para trazabilidad futura
  - Dialog mejorado: header custom con X, layout p-0 + flex + scroll, Resultado y Tipo de Solución en grid 2 columnas

#### Archivos modificados
- `components/corrective/failures/CreateIncidentDialog.tsx` — Rediseño completo a stepper wizard + multi-select combobox
- `components/corrective/failures/FailureQuickReportDialog.tsx` — Convertido a stepper wizard de 3 pasos
- `components/work-orders/AssignAndPlanDialog.tsx` — Mejorado con combobox, secciones, preview de resultado
- `components/corrective/work-orders/GuidedCloseDialog.tsx` — Sección de corrección de diagnóstico + mejoras layout
- `app/api/failure-occurrences/quick-report/route.ts` — Nuevos campos en schema + lógica de controles

---

### ✅ AgendaV2 — Mejoras y nuevas vistas (sesión 3)

**Bugs corregidos:**
- `CreateGroupModal`: eliminado botón X duplicado del `DialogContent` de shadcn/ui (clase `[&>button]:hidden`)
...
**Nuevos componentes:**
- `ReportingView.tsx` — Vista de reporting con Recharts: 4 KPIs (total, completadas, en progreso, vencidas), gráfico de actividad 7 días (barras), distribución por estado (horizontal), distribución por prioridad (pie donut), top assignees con barra de progreso, tasa de completación con color dinámico.
- `PortfolioView.tsx` — Vista portfolio: proyectos con tarjetas animadas (MiniDonut Recharts, breakdown de estado, avatares de miembros, hover con shadow y borde de color), grupos simples como lista. Click en tarjeta navega al board filtrado por ese grupo.

**Mejoras a componentes existentes:**
- `InboxView.tsx` — Rediseño completo: filtros tabs (Todas/Pendientes/Completadas), agrupación por fecha (Hoy/Ayer/fecha), panel derecho con header de asignador + meta row (priority chip + status dot + fecha), sección de adjuntos mejorada, footer de acciones rápidas. Semántica correcta: inbox = tareas asignadas A mí.
- `AgendaV2Sidebar.tsx` — Agregada sección "Tareas Fijas" (muestra tareas del día actual), NAV_ITEMS habilitados para Reporting + Portfolio (antes `disabled: true`), `ViewMode` extendido a 5 valores.
- `AgendaV2Page.tsx` — `ViewMode` extendido, renders de `ReportingView` y `PortfolioView` con props correctas.
- `index.ts` — Re-exports de `ReportingView` y `PortfolioView`.
...
### Sesión 3 — Tareas Fijas navegable + animaciones Synchro

**Problema 1:** "Tareas Fijas" en sidebar era solo display estático — no navegable.
**Problema 2:** Animaciones Synchro del plan no estaban completas.

**Soluciones implementadas:**

#### FixedTasksView (`components/agendav2/FixedTasksView.tsx`) — NUEVO
- Vista lista-style (como la agenda común, no kanban V2)
- Secciones por status: Por hacer / En progreso / En revisión / Completado
- Header por sección con icono + color + count chip + colapso animado
- Columna headers: Tarea / Categoría / Prioridad / Fecha / Asignado
- Filas `TaskRow` con: staggered entry animation, hover highlight, priority chip, date con color overdue/today, avatar assignee
- "Agregar tarea" por sección → abre CreateTaskModal
- Skeleton loading con `animate-pulse`

#### AgendaV2Page.tsx — actualizado
- `ViewMode` extendido a incluir `'fixed-tasks'`
- `VIEW_LABEL` actualizado con `'fixed-tasks': 'Tareas Fijas'`
- Import `FixedTasksView`
- **View transition animation**: `@keyframes view-fade-in { from opacity:0 translateY(7px) }` — el contenedor principal tiene `key={view-viewAnimKey}` para triggear la animación CSS al cambiar de vista
- Render `FixedTasksView` cuando `view === 'fixed-tasks'`

#### AgendaV2Sidebar.tsx — actualizado
- `ViewMode` local extendido con `'fixed-tasks'`
- Header "Tareas Fijas" cambiado de `<div>` estático a `<button>` clickeable que llama `onViewChange('fixed-tasks')`
- Estilo activo: fondo `#EDEDED`, color `#050505`, igual que los nav items principales
- Cada tarea en la preview también es un `<button>` que navega a `fixed-tasks`

#### TaskDetailPanel.tsx — animación tabs
...
- `panel-slide-in` (TaskDetailPanel): translateX(48px)→0 al abrir panel
- `tab-fade-in` (TaskDetailPanel): opacity+translateY al cambiar tabs ← NUEVO
- `view-fade-in` (AgendaV2Page): opacity+translateY al cambiar de vista ← NUEVO
- `row-slide-in` (FixedTasksView): staggered rows al cargar ← NUEVO
...
### Sesión 4 — FixedTasksView reemplazado por kanban de agendav1

**Problema:** El usuario indicó que "Tareas Fijas tiene que ser kanban como en tareas fijas de agendav1".
La implementación anterior era una vista lista por status, no el kanban por frecuencia.

**Solución:** Reescribir `FixedTasksView.tsx` para ser un wrapper auto-contenido que:
1. Usa `useFixedTasks()` hook → llama `/api/fixed-tasks` (mismo que agendav1)
2. Renderiza `TareasFijasTab` (KPIs + kanban por frecuencia: diaria/semanal/quincenal/mensual/trimestral/semestral/anual)
3. Usa los mismos modales que agendav1:
   - `CreateFixedTaskModal` → crear/editar tarea fija
   - `FixedTaskDetailModal` → ver detalle con tabs (info/instructivos/historial)
   - `TaskExecutionModal` → ejecutar/completar tarea con notas y adjuntos
4. CRUD completo: handleTaskClick, handleEditTask, handleDeleteTask, handleCreateTask, handleExecuteTask, handleCreateSubmit, handleExecutionComplete

**AgendaV2Page.tsx:** `<FixedTasksView />` ahora se renderiza sin props (self-contained).

**Kanban columnas:** Diarias · Semanales · Quincenales · Mensuales · Trimestrales · Semestrales · Anuales

**Archivos modificados:** `FixedTasksView.tsx` (reescrito), `AgendaV2Page.tsx` (sin props en render)
**TypeScript:** 0 errores
