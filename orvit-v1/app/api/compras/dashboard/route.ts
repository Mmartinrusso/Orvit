...
async function getComprasPorMes(companyId: number, meses: number, viewMode: ViewMode): Promise<Array<{ mes: string; total: number }>> {
  ...
  const months = Array.from({ length: meses }, (_, i) => {
    const targetDate = new Date(now.getFullYear(), now.getMonth() - (meses - 1 - i), 1);
    const startOfMonth = new Date(targetDate.getFullYear(), targetDate.getMonth(), 1);
    const endOfMonth = new Date(targetDate.getFullYear(), targetDate.getMonth() + 1, 0, 23, 59, 59);
    return { targetDate, startOfMonth, endOfMonth };
  });

  // Run all monthly queries in parallel instead of sequentially
