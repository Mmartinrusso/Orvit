...
// Obtener docType predominante de un item
...
// POST - Aprobar y confirmar ajuste (todo en uno)
  ...
  try {
    ...
    if (ajuste.estado !== 'PENDIENTE_APROBACION') {
      return NextResponse.json(
        { error: `Solo se pueden aprobar ajustes en estado PENDIENTE_APROBACION. Estado actual: ${ajuste.estado}` },
        { status: 400 }
      );
    }
    ...
    // Aprobar Y confirmar en una sola transacción
    const resultado = await prisma.$transaction(async (tx) => {
      ...
      // Actualizar ajuste a CONFIRMADO con info de aprobación
    ...
    await logStateChange({
      ...
      companyId,
      userId: user.id,
      motivo: 'Aprobación y confirmación de ajuste de inventario'
    });
    ...
    return NextResponse.json({
      ...ajusteCompleto,
      message: 'Ajuste aprobado y confirmado. Stock actualizado correctamente.'
    });
  } catch (error: any) {
    console.error('Error approving ajuste:', error);
    return NextResponse.json(
      { error: error.message || 'Error al aprobar el ajuste' },
      { status: 500 }
    );
  }
}
