...
const COLORS = {
  ...
  border: [200, 200, 200] as [number, number, number]     // Gris para bordes
};
...
function generateControlHorizontalPDF(
    ...
    data.preventiveMaintenances.forEach(maint => {
      ...
      if (machine && filters.machineIds.includes(machine.id)) {
        controlData.push([
          ...
          formatFrequency(maint.frequency, maint.frequencyUnit),
          formatPriority(maint.priority),
          '________________', // Realizado por
  ...
  (doc as any).autoTable({
    startY: yPosition,
    head: [['✓', 'Máquina', 'Título', 'Tipo', 'Frecuencia/Estado', 'Prioridad', 'Realizado por', 'Firma', 'Fecha']],
    body: controlData,
    theme: 'grid',
    styles: {
      fontSize: 8,
    ...
    columnStyles: {
      ...
      2: { cellWidth: 50 },  // Título
      3: { cellWidth: 22 },  // Tipo
      4: { cellWidth: 35 },  // Frecuencia/Estado
      5: { cellWidth: 22 },  // Prioridad
      6: { cellWidth: 45 },  // Realizado por
      7: { cellWidth: 35 },  // Firma
      8: { cellWidth: 25 }   // Fecha
    },
  ...
  // Sección de firmas de supervisores
  yPosition += 15;
  
  doc.setTextColor(...COLORS.black);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.text('SUPERVISIÓN Y APROBACIÓN:', 20, yPosition);
  ...
  doc.text('Nombre: ________________________________', 20, yPosition + 8);
  doc.text('Firma: ________________________________', 20, yPosition + 16);
  doc.text('Fecha: ________________________________', 20, yPosition + 24);
  
  // Supervisor del Supervisor
  doc.text('Supervisor del Supervisor:', pageWidth - 150, yPosition);
  doc.text('Nombre: ________________________________', pageWidth - 150, yPosition + 8);
  doc.text('Firma: ________________________________', pageWidth - 150, yPosition + 16);
  doc.text('Fecha: ________________________________', pageWidth - 150, yPosition + 24);
  
  // Footer con fecha de generación
  doc.setTextColor(...COLORS.gray);
  doc.setFontSize(8);
  doc.text(`Generado el ${new Date().toLocaleDateString('es-ES')}`, 20, pageHeight - 10);
...
function generateControlVerticalPDF(
  ...
  if (filters.maintenanceTypes.includes('PREVENTIVE')) {
    ...
    data.preventiveMaintenances.forEach(maint => {
      ...
      if (machine && filters.machineIds.includes(machine.id)) {
        ...
        // Campos para realizado por, firma y fecha
        doc.setFontSize(7);
        doc.text('Realizado por: _________________ Firma: _________________ Fecha: _______', 30, yPosition + 12);
  ...
  // Supervisor del Supervisor
  doc.text('Supervisor del Supervisor:', 20, yPosition);
  doc.text('Nombre: ________________________________', 20, yPosition + 8);
  doc.text('Firma: ________________________________', 20, yPosition + 16);
  doc.text('Fecha: ________________________________', 20, yPosition + 24);
