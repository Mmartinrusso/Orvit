...
import { WorkstationsFilters, SortOption } from './useWorkstationsFilters';
import { cn } from '@/lib/utils';

interface WorkstationsToolbarProps {
  totalCount: number;
  filteredCount: number;
  filters: WorkstationsFilters;
  onFiltersChange: (filters: WorkstationsFilters) => void;
  onRefresh: () => void;
...
export function WorkstationsToolbar({
  totalCount,
  filteredCount,
  filters,
  onFiltersChange,
  ...
  availableSectores = [],
  className,
}: WorkstationsToolbarProps) {
  const hasFilters = filteredCount !== totalCount;

  const handleFilterChange = (key: keyof WorkstationsFilters, value: any) => {
  ...
  const sortOptions: Array<{ value: SortOption; label: string }> = [
    ...
    { value: 'instructives-desc', label: 'Instructivos (mayor)' },
    { value: 'machines-desc', label: 'Máquinas (mayor)' },
    { value: 'recent-desc', label: 'Recientes' },
  ];
  ...
  const activeFilterChips = React.useMemo(() => {
    ...
    if (filters.hasInstructives !== 'all') {
      chips.push({
        label: `Instructivos: ${filters.hasInstructives === 'yes' ? 'Sí' : 'No'}`,
        key: 'hasInstructives',
        onRemove: () => handleFilterChange('hasInstructives', 'all'),
      });
    }
    
    if (filters.hasMachines !== 'all') {
      chips.push({
        label: `Máquinas: ${filters.hasMachines === 'yes' ? 'Sí' : 'No'}`,
        key: 'hasMachines',
        onRemove: () => handleFilterChange('hasMachines', 'all'),
      });
    }
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            >
              {hasFilters ? (
                ...
                <span>{totalCount} {totalCount === 1 ? 'puesto' : 'puestos'}</span>
              )}
            </Badge>
          </div>
      ...
      <div className="px-4 md:px-6">
          ...
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            ...
            <Button
              ...
              onClick={() => {
                ...
                handleFilterChange('hasInstructives', 'all');
                handleFilterChange('hasMachines', 'all');
        ...
        {hasFilters && (
          <p className="text-xs text-muted-foreground mt-2">
            Mostrando {filteredCount} de {totalCount} puestos
          </p>
        )}
      </div>
    </div>
