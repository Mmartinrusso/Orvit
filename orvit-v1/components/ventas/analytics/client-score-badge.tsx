...
export function ClientScoreBadge({ clientId, showTooltip = true, size = 'md' }: ClientScoreBadgeProps) {
  ...
  const getColorClass = () => {
    if (data.score >= 90) return 'bg-success-muted text-success hover:bg-success-muted/80';
    if (data.score >= 75) return 'bg-info-muted text-info-muted-foreground hover:bg-info-muted/80';
    if (data.score >= 60) return 'bg-warning-muted text-warning-muted-foreground hover:bg-warning-muted/80';
