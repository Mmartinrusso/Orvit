...
import { ClipboardCheck, Clock, AlertTriangle, Bell, CheckCircle } from 'lucide-react';
...
export function MyControlsTimelineWidget({
  companyId,
  userId,
  onRemove,
  isEditMode,
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['my-controls-pending', companyId, userId],
    queryFn: async () => {
  ...
  return (
    <WidgetWrapper
      title="Mis Controles"
      icon={<ClipboardCheck className="h-4 w-4 text-info-muted-foreground" />}
      isLoading={isLoading}
      isError={isError}
      onRefresh={() => refetch()}
      onRemove={onRemove}
    ...
    >
      {controls.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-full py-8 text-center">
          ...
          <p className="text-xs text-muted-foreground">Sin controles pendientes</p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground truncate">
                    {control.solutionApplied?.machineName || 'Sin máquina'}
                    {control.solutionApplied?.failureTitle && ` — ${control.solutionApplied.failureTitle}`}
                  </div>
                </div>
