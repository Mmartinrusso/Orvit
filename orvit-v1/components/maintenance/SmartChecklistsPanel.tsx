...
export default function SmartChecklistsPanel({
 ...
 const queryKey = ['smart-checklists', companyId, sectorId, machineId];
 const { data: smartChecklists = [], isLoading: loading } = useQuery({
 queryKey,
 queryFn: async () => {
 ...
 if (!data.success) throw new Error(data.error || 'Error fetching smart checklists');
 return data.checklists as SmartChecklist[];
 },
 staleTime: 2 * 60 * 1000,
 });

 const invalidateChecklists = () => queryClient.invalidateQueries({ queryKey });

 // Inicializar selecciones cuando cambian los checklists
 useEffect(() => {
 if (smartChecklists.length > 0) {
 const initialSelections: {[key: string]: number[]} = {};
 smartChecklists.forEach((checklist: SmartChecklist) => {
 ...
 const colors = {
 'CORRECTIVE': 'bg-destructive/10 text-destructive border-destructive/30',
 'DAILY': 'bg-info-muted text-info-muted-foreground border-info-muted',
 'WEEKLY': 'bg-success-muted text-success-muted-foreground border-success-muted',
 'BIWEEKLY': 'bg-warning-muted text-warning-muted-foreground border-warning-muted',
 'MONTHLY': 'bg-primary/10 text-primary border-primary/30',
 'QUARTERLY': 'bg-info-muted text-info-muted-foreground border-info-muted',
 'SEMIANNUAL': 'bg-primary/5 text-primary border-primary/20',
 'ANNUAL': 'bg-muted text-foreground border-border'
 };
 return colors[frequency] || 'bg-muted text-foreground border-border';
 };
