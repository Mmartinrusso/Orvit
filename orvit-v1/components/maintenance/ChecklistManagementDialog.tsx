...
export default function ChecklistManagementDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (selectedMaintenances.length > 0 && usePhases) {
        console.log('🔄 Dividiendo automáticamente por fases...');
        console.log('📊 Mantenimientos seleccionados para crear fases:', selectedMaintenances);
        console.log('📊 Fases actuales antes de recrear:', phasesToUse.length, 'fases');
        ...
        console.log('📊 Mantenimientos encontrados para crear fases:', selectedMaintenanceData.length, 'mantenimientos');
        selectedMaintenanceData.forEach((m, idx) => {
        ...
        console.log('🔄 Iniciando distribución de', selectedMaintenanceData.length, 'mantenimientos en fases...');
        selectedMaintenanceData.forEach((maintenance, maintIndex) => {
          console.log(`📦 Procesando mantenimiento ${maintIndex + 1}/${selectedMaintenanceData.length}: "${maintenance.title}" (ID: ${maintenance.id})`);
          ...
          console.log(`⏱️ Mantenimiento "${maintenance.title}":`, {
            timeValue: maintenance.timeValue,
            timeUnit: maintenance.timeUnit,
            estimatedHours: maintenance.estimatedHours,
            calculatedMinutes: estimatedTimeInMinutes
          });
          ...
          console.log(`⏱️ [createPhasesAutomatically] Item creado para "${maintenance.title}":`, {
            timeValue: maintenance.timeValue,
            timeUnit: maintenance.timeUnit,
            estimatedHours: maintenance.estimatedHours,
            calculatedMinutes: estimatedTimeInMinutes
          ...
          if (maintenance.executionWindow) {
            console.log(`🔍 Mantenimiento "${maintenance.title}" - Ventana de ejecución: ${maintenance.executionWindow}`);
          ...
          console.log(`✅ Mantenimiento "${maintenance.title}" (ID: ${maintenance.id}) asignado a Fase ${phaseIndex + 1}`);
          console.log(`📊 Items en fase ${phaseIndex + 1} antes de agregar:`, phases[phaseIndex].items.length);
          ...
          console.log(`📊 Items en fase ${phaseIndex + 1} después de agregar:`, phases[phaseIndex].items.length);
          console.log(`📋 Items actuales en fase ${phaseIndex + 1}:`, phases[phaseIndex].items.map(i => `${i.title} (ID: ${i.maintenanceId})`).join(', '));
        });

        // Filtrar solo fases con elementos
        const phasesWithItems = phases.filter(phase => phase.items.length > 0);
        
        // Log detallado de todas las fases antes de usar
        console.log('📊 Fases creadas después de distribuir:', phasesWithItems.length, 'fases');
        phasesWithItems.forEach((phase, idx) => {
          console.log(`  Fase ${idx + 1} (${phase.name}): ${phase.items.length} items`);
          phase.items.forEach((item, itemIdx) => {
            console.log(`    Item ${itemIdx + 1}: ${item.title} (maintenanceId: ${item.maintenanceId})`);
          });
        });
        ...
        console.log('📊 Verificación: Total items en fases:', totalItemsInPhases, 'vs Total items seleccionados:', selectedMaintenanceData.length);
      ...
      console.log('⏱️ Tiempo total estimado calculado:', estimatedTotalTime, 'minutos');
      ...
      console.log('📤 Preparando payload para guardar:');
      console.log('  - usePhases:', usePhases);
      console.log('  - Fases a guardar:', finalPhases.length, 'fases');
      finalPhases.forEach((phase, idx) => {
        console.log(`    Fase ${idx + 1} (${phase.name}): ${phase.items.length} items`);
        phase.items.forEach((item, itemIdx) => {
          console.log(`      Item ${itemIdx + 1}: ${item.title} (maintenanceId: ${item.maintenanceId})`);
        });
      });
      console.log('  - Items directos a guardar:', selectedMaintenances.length, 'items');
      ...
      const payload = {
        ...
        items: selectedMaintenances.map(id => {
          ...
          if (maintenance) {
            ...
            console.log(`🔍 [handleSubmit] Calculando tiempo para mantenimiento "${maintenance.title}":`, {
              estimatedMinutes: maintenance.estimatedMinutes,
              estimatedTimeType: maintenance.estimatedTimeType,
              timeValue: maintenance.timeValue,
              timeUnit: maintenance.timeUnit,
              unidadMovilId: maintenance.unidadMovilId,
            ...
            if (maintenance.estimatedMinutes && maintenance.estimatedTimeType) {
              if (maintenance.estimatedTimeType === 'HOURS') {
                ...
                console.log(`✅ [handleSubmit] Usando campos nuevos (HOURS): ${maintenance.estimatedMinutes} * 60 = ${estimatedTime} minutos`);
              } else if (maintenance.estimatedTimeType === 'MINUTES') {
                estimatedTime = maintenance.estimatedMinutes;
                console.log(`✅ [handleSubmit] Usando campos nuevos (MINUTES): ${maintenance.estimatedMinutes} minutos`);
              }
            } else if (maintenance.timeValue && maintenance.timeUnit) {
              ...
              if (maintenance.timeUnit === 'HOURS') {
                ...
                console.log(`⚠️ [handleSubmit] Usando campos antiguos (HOURS): ${maintenance.timeValue} * 60 = ${estimatedTime} minutos`);
              } else if (maintenance.timeUnit === 'MINUTES') {
                estimatedTime = maintenance.timeValue;
                console.log(`⚠️ [handleSubmit] Usando campos antiguos (MINUTES): ${maintenance.timeValue} minutos`);
              }
            }
            
            console.log(`⏱️ [handleSubmit] Tiempo final calculado para "${maintenance.title}": ${estimatedTime} minutos`);
      ...
      console.log('📚 Instructivos en el payload:', {
        count: instructives.length,
        instructives: instructives.map(inst => ({
          title: inst.title,
          contentLength: inst.content?.length || 0
        }))
      });
      
      console.log('📊 Fases finales en payload:', payload.phases.length, 'fases');
      payload.phases.forEach((phase: any, idx: number) => {
        console.log(`  Fase ${idx + 1} (${phase.name}): ${phase.items?.length || 0} items`);
        phase.items?.forEach((item: any, itemIdx: number) => {
          console.log(`    Item ${itemIdx + 1}: ${item.title} (maintenanceId: ${item.maintenanceId})`);
        });
      });

      console.log('📤 Enviando datos del checklist:', JSON.stringify(payload, null, 2));
      console.log('🔧 Mantenimientos seleccionados:', selectedMaintenances);
      console.log('📋 Fases a guardar:', formData.phases);
      console.log('📊 Detalle de fases antes de guardar:');
      formData.phases?.forEach((phase, index) => {
        console.log(`  Fase ${index + 1} (${phase.name}): ${phase.items?.length || 0} items`);
        phase.items?.forEach((item, itemIndex) => {
          console.log(`    Item ${itemIndex + 1}: ${item.title} (maintenanceId: ${item.maintenanceId})`);
        });
      });
  ...
  const divideIntoPhases = () => {
    ...
    selectedMaintenanceData.forEach(maintenance => {
      ...
      console.log(`✅ Mantenimiento "${maintenance.title}" asignado a Fase ${phaseIndex + 1}`);
  ...
  const createPhasesAutomatically = () => {
    ...
    if (!usePhases) {
      console.log('⛔ Fases desactivadas - No creando fases');
      setFormData(prev => ({ ...prev, phases: [] }));
      return;
    }
    ...
    if (hasUnidadMovil) {
      console.log('🚛 Checklist contiene unidades móviles - No organizando por fases');
      setFormData(prev => ({ ...prev, phases: [] }));
      return;
    }

    console.log('🔄 Creando fases automáticamente...');
    ...
    // Log detallado de todas las fases después de distribuir
    console.log('📋 Fases después de distribuir mantenimientos:');
    phases.forEach((phase, index) => {
      console.log(`  Fase ${index + 1} (${phase.name}): ${phase.items.length} items`);
      phase.items.forEach((item, itemIndex) => {
        console.log(`    Item ${itemIndex + 1}: ${item.title} (ID: ${item.maintenanceId})`);
      });
    });
    ...
    // Log para debug
    console.log('📊 Total items procesados:', selectedMaintenanceData.length);
    console.log('📊 Total items en fases después de filtrar:', phasesWithItems.reduce((total, phase) => total + phase.items.length, 0));
  ...
  const toggleMaintenanceSelection = (maintenance: any) => {
    ...
    if (selectedMaintenances.includes(maintenance.id)) {
      ...
      if (editingChecklist) {
        ...
        console.log(`⏱️ [toggleMaintenanceSelection] Mantenimiento "${maintenance.title}":`, {
          timeValue: maintenance.timeValue,
          timeUnit: maintenance.timeUnit,
          estimatedHours: maintenance.estimatedHours,
          calculatedMinutes: estimatedTimeInMinutes
        ...
        console.log(`⏱️ [toggleMaintenanceSelection] Item creado para "${maintenance.title}":`, {
          timeValue: maintenance.timeValue,
          timeUnit: maintenance.timeUnit,
          estimatedHours: maintenance.estimatedHours,
          calculatedMinutes: estimatedTimeInMinutes
  ...
  const removeMaintenanceFromPhase = (maintenanceId: string, phaseId: string) => {
    ...
    console.log('🗑️ Removiendo mantenimiento de fase:', maintenanceId, 'de fase:', phaseId);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent size="xl" className="p-0 flex flex-col overflow-hidden">
        {/* HEADER STICKY */}
                        ...
                        <div className="space-y-2 p-2">
                          {filteredMaintenances.map((maintenance: any) => {
                            ...
                            return (
                              <div
                                key={maintenance.id}
                                className={`p-2.5 border rounded-md cursor-pointer transition-colors flex items-start gap-3 ${
                                  isSelected
                                    ? 'border-primary bg-primary/5'
                                    : 'border-border hover:border-primary/50 hover:bg-muted/50'
                                }`}
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex-1 min-w-0">
                                ...
                                <div 
                                  ...
                                  dangerouslySetInnerHTML={{ __html: instructive.content.replace(/<[^>]*>/g, '').substring(0, 100) + '...' }}
                                />
                              </div>
                              <Button
                                type="button"
                                ...
                                className="text-red-600 hover:bg-red-100 h-7 w-7 p-0 shrink-0 ml-2"
                              >
                                <X className="h-3.5 w-3.5" />
                              </Button>
                            </div>
