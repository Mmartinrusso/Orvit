...
export default function MaintenanceDialog({
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      case 'PREVENTIVE': return <Settings className="h-6 w-6 text-green-600" />;
      case 'CORRECTIVE': return <Wrench className="h-6 w-6 text-red-600" />;
      case 'PREDICTIVE': return <AlertTriangle className="h-6 w-6 text-orange-600" />;
      default: return <Settings className="h-6 w-6 text-green-600" />;
    }
  };
                      ...
                      <SelectTrigger>
                        <SelectValue>
                          <div className="flex items-center gap-2">
                            <div className={`w-3 h-3 rounded-full ${getPriorityDot(formData.priority)}`}></div>
                  ...
                  <div className="mt-6 p-4 bg-muted rounded-lg">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {formData.selectedSubcomponents?.map((subcomponentId: string) => {
                        ...
                        return subcomponent ? (
                          <Badge key={subcomponentId} variant="outline" className="bg-green-50 text-green-700">
                            {subcomponent.name}
                          </Badge>
                        ) : null;
                      })}
                    ...
                    <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                      <div>
                        ...
                        {instructive.description && (
                          <p className="text-sm text-gray-500">{instructive.description}</p>
                        )}
                      </div>
                      <Button
                        variant="outline"
