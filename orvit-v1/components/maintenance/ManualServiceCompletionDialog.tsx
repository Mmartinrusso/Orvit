...
export default function ManualMaintenanceCompletionDialog({
 ...
 case 'today': return 'bg-warning-muted text-warning-muted-foreground border-warning-muted';
 case 'upcoming': return 'bg-success-muted text-success border-success-muted';
 ...
 { id: 'overdue', label: 'Vencidos', count: statusCounts.overdue, color: 'text-destructive' },
 { id: 'today', label: 'Hoy', count: statusCounts.today, color: 'text-warning-muted-foreground' },
 { id: 'upcoming', label: 'Próximos', count: statusCounts.upcoming, color: 'text-success' }
 ].map((filter) => (
 <button
 key={filter.id}
 type="button"
 ...
 <span className="text-xs font-medium text-info-muted-foreground">Aplicar a todos</span>
 </div>
 ...
 {/* p-0, gap-0: custom two-panel layout with manual spacing */}
 <DialogContent
 size="xl"
 className={cn(
 "p-0 gap-0",
