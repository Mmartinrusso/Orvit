        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">{machine.totalCount} mantenimientos</p>
            </div>
          </div>
          <div className="text-right flex flex-col items-end gap-1">
            {machine.metaRightTop && (
              <span className="text-xs text-gray-500">{machine.metaRightTop}</span>
            )}
      ...
      {machine.groups.map((group, gIdx) => (
        <div key={gIdx} className="mb-4 last:mb-0">
          <div className="flex items-center gap-2 mb-2">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={cn('text-gray-500', group.tone === 'green' && 'text-green-600', group.tone === 'red' && 'text-red-600', group.tone === 'amber' && 'text-amber-600')}>
              <path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"></path>
            </svg>
            <span className="font-semibold text-gray-700 text-sm">{group.title} ({group.count})</span>
          </div>
          ...
          <div className={cn('space-y-2', compact && 'space-y-1')}>
            {group.items.map((item) => (
              <div
                key={item.id}
                className={cn(
                  'bg-white border rounded-lg border-l-4',
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className={cn('font-semibold text-gray-900', compact ? 'text-xs' : 'text-sm')}>
                      {item.name}
                    </div>
                    {!compact && item.description && (
                      <p className="text-xs text-gray-600 mt-1 whitespace-pre-line line-clamp-2">
                        {item.description}
                      </p>
                    )}
                    <div className={cn('flex items-center gap-3 text-gray-500 flex-wrap', compact ? 'mt-1 text-[10px]' : 'mt-2 text-xs')}>
  ...
  return createPortal(
    <>
      ...
      <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 pointer-events-none">
        <div 
          className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[92vh] overflow-hidden pointer-events-auto flex flex-col"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header */}
          <div className="px-6 py-4 border-b bg-gray-50/80 flex-shrink-0">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-xl font-bold text-gray-900">{report.companyName}</h2>
                <p className="text-sm text-gray-600 mt-0.5">{report.title}</p>
                <p className="text-xs text-gray-400 mt-1">Generado el {report.generatedAt}</p>
              </div>
            ...
            {report.appliedFilters.length > 0 && (
              <div className="bg-gray-50 rounded-lg p-3 mb-5">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-semibold text-gray-700">Filtros aplicados</span>
                  {hasMoreFilters && (
                    <button 
                ...
                <div className="flex flex-wrap gap-2">
                  {visibleFilters.map((filter, idx) => (
                    <span
                      key={idx}
                      className="inline-flex items-center px-2.5 py-1 rounded-full text-xs bg-white border text-gray-700"
                    >
                      <span className="font-medium text-gray-500 mr-1">{filter.label}:</span>
                      {filter.value}
                    </span>
                  ))}
                </div>
          ...
          {/* Footer */}
          <div className="px-6 py-3 border-t bg-gray-50/80 flex items-center justify-between text-xs text-gray-500 flex-shrink-0">
            <span>Listado generado automáticamente • Sistema de Mantenimiento</span>
