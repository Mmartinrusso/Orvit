...
export function SolicitudNcaDetalleModal({
  ...
  const handleEnviar = async () => {
    if (!confirm('¿Enviar esta solicitud al proveedor?')) return;
    try {
      setLoading(true);
  ...
  const handleRegistrarNca = async () => {
    try {
      ...
      alert('Error al registrar la NCA');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {solicitud.montoAprobado != null && (
              <div className="p-4 bg-muted/30 rounded-lg">
                ...
                <p className={`text-2xl font-bold ${
                  solicitud.montoAprobado < solicitud.montoSolicitado ? 'text-orange-600' : 'text-green-600'
                }`}>
