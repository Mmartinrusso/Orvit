...
const CATEGORY_COLORS: Record<string, { bg: string; text: string }> = {
 APPROVAL: { bg: 'bg-success-muted', text: 'text-success' },
 REJECTION: { bg: 'bg-destructive/10', text: 'text-destructive' },
 CANCELLATION: { bg: 'bg-muted', text: 'text-foreground' },
 MODIFICATION: { bg: 'bg-info-muted', text: 'text-info-muted-foreground' },
 RESOLUTION: { bg: 'bg-purple-100', text: 'text-purple-700' },
 OPERATIONAL: { bg: 'bg-warning-muted', text: 'text-warning-muted-foreground' },
};
...
export function ReasonCodeSelector({
 ...
 rows={compact ? 2 : 3}
 disabled={disabled}
 className={cn(!text && required && "border-destructive/30")}
 />
 {!text && required && (
