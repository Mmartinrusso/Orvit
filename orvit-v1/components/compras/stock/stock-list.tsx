...
const CRITICIDAD_CONFIG: Record<string, { label: string; color: string }> = {
  CRITICO: { label: 'Critico', color: 'bg-red-100 text-red-700 border-red-200' },
  A: { label: 'A', color: 'bg-orange-100 text-orange-700 border-orange-200' },
  B: { label: 'B', color: 'bg-yellow-100 text-yellow-700 border-yellow-200' },
  C: { label: 'C', color: 'bg-gray-100 text-gray-600 border-gray-200' },
};
...
export function StockList() {
  ...
  // State
  const [stockItems, setStockItems] = useState<StockLocation[]>([]);
  ...
  // Items sin deposito
  const [stockSinDeposito, setStockSinDeposito] = useState<StockSinDeposito[]>([]);
  const [loadingSinDeposito, setLoadingSinDeposito] = useState(false);
  ...
  useEffect(() => {
    loadWarehouses();
    loadStockSinDeposito();
  }, []);

  // Load data when filters change or viewMode changes (juego de tecla)
  useEffect(() => {
    loadStock();
    loadKPIs();
  }, [warehouseFilter, alertaFilter, page, viewMode]);
  ...
  const loadWarehouses = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setWarehouses(Array.isArray(data) ? data.filter((w: Warehouse) => !w.isTransit) : []);
      }
    } catch (error) {
  ...
  const loadKPIs = async () => {
    try {
      const response = await fetch('/api/compras/stock/kpis');
      if (response.ok) {
  ...
  const loadStockSinDeposito = async () => {
    setLoadingSinDeposito(true);
    try {
      const response = await fetch('/api/compras/stock/sin-deposito');
      if (response.ok) {
        const result = await response.json();
        setStockSinDeposito(result.data || []);
      }
    } catch (error) {
      console.error('Error loading stock sin deposito:', error);
    } finally {
      setLoadingSinDeposito(false);
    }
  };
  ...
  const loadStock = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/compras/stock/ubicaciones?${params.toString()}`);
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    await Promise.all([loadStock(), loadKPIs()]);
    setRefreshing(false);
  ...
  const getStockStatusBadge = (item: StockLocation) => {
    ...
    if (cantidad <= 0) {
      return <Badge className="bg-red-100 text-red-700 border-red-200 text-[10px]">Sin Stock</Badge>;
    }
    if (minimo > 0 && (disponible + enCamino) < minimo) {
      return <Badge className="bg-orange-100 text-orange-700 border-orange-200 text-[10px]">Bajo</Badge>;
    }
    if (maximo > 0 && cantidad > maximo) {
      return <Badge className="bg-blue-100 text-blue-700 border-blue-200 text-[10px]">Exceso</Badge>;
    }
    return <Badge className="bg-green-100 text-green-700 border-green-200 text-[10px]">OK</Badge>;
  };
      ...
      <div className="px-6 py-4">
        ...
        {kpis && (
          <div className="grid grid-cols-5 gap-3 mb-4">
            <Card
              className={`cursor-pointer transition-all ${alertaFilter === 'all' ? 'ring-2 ring-primary' : ''}`}
            ...
            <Card
              className={`cursor-pointer transition-all ${alertaFilter === 'bajo' ? 'ring-2 ring-primary' : ''}`}
            ...
            >
              <CardContent className="p-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xl font-bold text-orange-600">{kpis.itemsBajoStock}</p>
            ...
            <Card className="cursor-pointer">
              <CardContent className="p-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xl font-bold text-red-600">{kpis.itemsSinStock}</p>
            ...
            <Card className="cursor-pointer">
              <CardContent className="p-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xl font-bold text-blue-600">{formatCurrency(kpis.valorEnTransito)}</p>
        ...
        {stockSinDeposito.length > 0 && (
          <div className="mb-4">
            <div
              className="flex items-center justify-between p-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-lg cursor-pointer hover:bg-amber-100 dark:hover:bg-amber-950/50 transition-colors"
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-xs text-muted-foreground">No se encontraron items</p>
            {hasActiveFilters && (
              ...
              <TableBody>
                {filteredStock.map((item) => {
                  ...
                  return (
                    <TableRow
                      key={item.id}
                      className={`hover:bg-muted/30 ${item.alertas.sinStock ? 'bg-red-50' : item.alertas.stockBajo ? 'bg-orange-50' : ''}`}
                    >
                      <TableCell>
                        <Checkbox
                      ...
                      <TableCell className="text-center">
                        {item.criticidad && CRITICIDAD_CONFIG[item.criticidad] ? (
                          <Badge className={`${CRITICIDAD_CONFIG[item.criticidad].color} text-[10px]`}>
