-- Fix ALL NULL values before O2C migration v3
-- Even more comprehensive - catches columns missed before
...
-- For templateId, we can't easily fix without knowing which template to use
-- Delete orphaned records or set to first available template
DO $$
DECLARE
  first_template_id INT;
BEGIN
  SELECT id INTO first_template_id FROM maintenance_checklists LIMIT 1;
  IF first_template_id IS NOT NULL THEN
    UPDATE work_order_checklists SET "templateId" = first_template_id WHERE "templateId" IS NULL;
  ELSE
    -- No templates exist, delete orphaned checklists
...
-- Tables sales_deliveries, sales_remitos, sales_invoices, client_payments,
-- client_ledger_entries, cheques may not exist yet - will be handled after migration
