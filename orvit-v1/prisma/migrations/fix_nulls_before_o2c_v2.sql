-- Fix ALL NULL values before O2C migration v2
-- Comprehensive script based on full error output
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- WORK_LOGS - Add updatedAt column if not exists and fill
-- ═══════════════════════════════════════════════════════════════════════════════
-- This will be handled by adding default to column in schema or manually
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='work_logs' AND column_name='updatedAt') THEN
    ALTER TABLE work_logs ADD COLUMN "updatedAt" TIMESTAMP(3);
  END IF;
END $$;
UPDATE work_logs SET "updatedAt" = COALESCE("createdAt", NOW()) WHERE "updatedAt" IS NULL;

-- ═══════════════════════════════════════════════════════════════════════════════
-- WORK_ORDER_CHECKLISTS - Add responses column if not exists and fill
-- ═══════════════════════════════════════════════════════════════════════════════
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='work_order_checklists' AND column_name='responses') THEN
    ALTER TABLE work_order_checklists ADD COLUMN responses JSONB;
  END IF;
END $$;
UPDATE work_order_checklists SET responses = '{}'::jsonb WHERE responses IS NULL;
UPDATE work_order_checklists SET "templateId" = 1 WHERE "templateId" IS NULL;
