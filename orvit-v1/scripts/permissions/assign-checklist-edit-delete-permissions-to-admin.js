...
// Permisos de editar y eliminar checklists que necesitamos asignar
const CHECKLIST_EDIT_DELETE_PERMISSIONS = [
  'editar_checklist',
  'eliminar_checklist'
];

async function assignChecklistEditDeletePermissionsToAdmin() {
  try {
    console.log(`🚀 Asignando permisos de editar y eliminar checklists al rol "${ROLE_NAME}" en todas las empresas...\n`);
    ...
    for (const company of companies) {
      ...
      try {
        ...
        for (const permissionName of CHECKLIST_EDIT_DELETE_PERMISSIONS) {
          try {
            // Buscar el permiso
    ...
    console.log(`🎯 Permisos por rol: ${CHECKLIST_EDIT_DELETE_PERMISSIONS.length}`);
...
assignChecklistEditDeletePermissionsToAdmin();
