...
async function main() {
  ...
  const allRoles = Array.from(new Set(allEmployees.map((e: any) => e.role).filter(Boolean)));
  console.log('=== Todos los roles únicos en la empresa ===');
  allRoles.forEach((r: string) => console.log(' -', r));
  ...
  for (const ws of workSectors) {
    ...
    employees.forEach((e: any) => console.log(`  - ${e.name} | ${e.role} | activo: ${e.active}`));
